...
In the following example we use the internal Sickle Cell Disease (SCD) data to find the rare phenotype. SCD is extremely rare so we use DBSCAN to initialise the VB GMM. We also use an informative prior for the mixing coefficient and stop iterations when the ELBO starts to reverse so that we stop when the minor (SCD) component is reached.
...
-   $\nu$, the degrees of freedom, ensures the Wishart $\Gamma$ function is well-defined and is restricted to one less than the number of mixture components.  This is usually defaulted to one plus the number of variables.
...
The prior for the GMM mixing coefficients, $\pi$ affects the expectation such that if $\alpha \longrightarrow 0$, then the posterior distribution will be influenced primarily by the data and $\alpha \longrightarrow \inf$ then the prior will have increased influence on the posterior.
...
```{r, echo=FALSE, out.width="30%", out.height="30%", fig.show='hold', fig.align='center', fig.cap="Figure 1: Posterior clustering with three alpha prior hyperparameter settings. Left; low alpha setting, right; high alpha setting and middle; a setting between low and high."}

knitr::include_graphics(normalizePath(c("alpha_1_271.jpg","alpha_30_237.jpg","alpha_70_202.jpg"), winslash = "/"), rel_path = FALSE)
```
...
```{r, echo=FALSE, out.width="30%", out.height="30%", fig.show='hold', fig.align='center', fig.cap="Figure 2: Posterior clustering with three different alpha vector prior hyperparameter settings. Left; equal alpha vectors, right; different alpha vectors and middle; two equal and two different alpha vectors."}
...
The $\beta$ prior has a significant effect on the posterior as shown in Figure 3.  Here, we show the effect of a very low value for $\beta$ and a very high value for $\beta$.

```{r, echo=FALSE, out.width="45%", out.height="30%", fig.show='hold', fig.align='center', fig.cap="Figure 3: Posterior clustering with two different beta vector prior hyperparameter settings. Left; low beta vectors, right; high beta vectors."}
...
The $W$ prior for the Normal-Wishart posterior also has a significant effect on the posterior as illustrated in Figure 4.


```{r, echo=FALSE, out.width="45%", out.height="30%", fig.show='hold', fig.align='center', fig.cap="Figure 4: Posterior clustering with two different W vector prior hyperparameter settings. Left; low W vectors, right; high W vectors."}
...
The following examples were used to generate Figures 2 to 4.
...
``` {r eval=F, echo=T}
  ...
  # Run with 4 presumed components for demolnstration purposes
  k = 4
...
```{r, echo=FALSE, out.width="30%", out.height="30%", fig.show='hold', fig.align='center', fig.cap="Figure 5: Posterior clustering of SCD Cohort using *kmeans* as initialiser.  With k=2 (left) the model cannot distinguish the rare SCD cluster from the majority class. With k=3 (middle), the model is starting to find the SCD cluster but there is still too much overlap so the cluster centroid is far from the true SCD cluster. We have to go all the way to k=10 (right) before the SCD cluster is identified."}

knitr::include_graphics(c("gmm-k2-clusters.jpg", "gmm-k3-clusters.jpg", "gmm-k10-clusters.jpg"))
```
</br>

#### VB GMM example using DBSCAN

Figure 6 is a single run using *dbscan* as the initialiser for the highly unbalanced and noisy *scd_cohort* data.  *dbscan* is very capable of finding the rare phenotype class and thus provides excellent starting positions for the VB GMM. This is at the cost of computational performance.  While *kmeans* runs in less than a second, *dbscan* takes about 16 seconds for these data.  Another issue with *dbscan* is that it cannot be limited to a k cluster size so it often returns more than k clusters when k is small e.g. k = 2 in our current phenotyping model.  We therefore need to merge clusters returned by dbscan before passing the correct number k cluster centroids to VB GMM.
...
In variational Inference, the prior is part of the Kullback-Leibler divergence between the approximate posterior $Q(\beta)$ and the true posterior $P(\beta)$:
